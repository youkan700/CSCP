...
void MZ1R13::initialize()
{
	...
	if(fio->Fopen(create_local_path(_T("KANJI2.ROM"     )), FILEIO_READ_BINARY) ||
	   fio->Fopen(create_local_path(_T("MZ1R13_KAN.ROM" )), FILEIO_READ_BINARY) ||
	   fio->Fopen(create_local_path(_T("MZ-1R13_KAN.ROM")), FILEIO_READ_BINARY)) {
		fio->Fread(kanji, sizeof(kanji), 1);
		fio->Fclose();
	}
	if(fio->Fopen(create_local_path(_T("DICT2.ROM"      )), FILEIO_READ_BINARY) ||
	   fio->Fopen(create_local_path(_T("MZ1R13_DIC.ROM" )), FILEIO_READ_BINARY) ||
	   fio->Fopen(create_local_path(_T("MZ-1R13_DIC.ROM")), FILEIO_READ_BINARY)) {
